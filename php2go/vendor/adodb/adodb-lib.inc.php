 ...
 * Parameterize SQL
 *
 * @param sql		SQL statement to parameterize
 * @return 			Prepared statement array
 */
function adodb_parametrize($sql) {
	$arr = array();
	$len = strlen($sql);
	$next_mark = 0;
	$next_type = 0;
	...
	$i = 0; $st = 0;
	while($i<$len) {
		// efficiently find the next special character
		$next_mark = $len; $next_type = 0;
		// find next double-quote mark
		if(($next_dquote!==false) && ($next_dquote<$i)) $next_dquote = strpos($sql,'"',$i);
		if(($next_dquote!==false) && ($next_dquote<$next_mark)) {
			$next_mark = $next_dquote;
			$next_type = 1;
		}
		// find next single-quote mark
		if(($next_squote!==false) && ($next_squote<$i)) $next_squote = strpos($sql,'\'',$i);
		if(($next_squote!==false) && ($next_squote<$next_mark)) {
			$next_mark = $next_squote;
			$next_type = 2;
		}
		// find next backtick mark
		if(($next_btick!==false) && ($next_btick<$i)) $next_btick = strpos($sql,'`',$i);
		if(($next_btick!==false) && ($next_btick<$next_mark)) {
			$next_mark = $next_btick;
			$next_type = 3;
		}
		// find next question mark
		if(($next_qmark!==false) && ($next_qmark<$i)) $next_qmark = strpos($sql,'?',$i);
		if(($next_qmark!==false) && ($next_qmark<$next_mark)) {
			$next_mark = $next_qmark;
			$next_type = 4;
		}
		// what was the next special character?
		switch($next_type) {
		case 0: // no more special characters in the string
			$arr[] = substr($sql,$st);
			$i = $len; $st = $len + 1;
			break;
		case 1: // next special character is a double-quote
			// seek the next unescaped double-quote
			$next = $next_mark + 1;
			while($next!==false) {
				$next = strpos($sql,'"',$next);
				if($next===false) {
					// unterminated quote.
					$arr[] = substr($sql,$st);
					$i = $len; $st = $len+1;
			...
			break;
		case 2: // next special character is a single-quote
			// seek the next unescaped single-quote
			$next = $next_mark + 1;
			while($next!==false) {
				$next = strpos($sql,'\'',$next);
				if($next===false) {
			...
			break;
		case 3: // next special character is a backtick
			// seek the next back-tick
			$next = $next_mark + 1;
			while($next!==false) {
				$next = strpos($sql,'`',$next);
				if($next===false) {
			...
			break;
		case 4: // next special character is a question mark
			$arr[] = substr($sql,$st,$next_mark - $st);
			$i = $next_mark + 1;
			$st = $i;
			break;
		}
	}
	if($st<$len) $arr[] = substr($sql,$st);
	if($st==$len) $arr[] = '';
	return $arr;
}
